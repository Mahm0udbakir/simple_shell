 ...
 * @shell_info: the shell information structure
 * @estr: string containing specified error type
 *
 * Return: Nothing
 */
void custom_print_error(shell_info_t *shell_info, char *estr)
{
    custom_eputs(shell_info->fname);
    custom_eputs(": ");
    custom_print_d(shell_info->line_count, STDERR_FILENO);
    custom_eputs(": ");
    custom_eputs(shell_info->argv[0]);
    custom_eputs(": ");
    custom_eputs(estr);
}
...
/**
 * custom_remove_comments - replaces first instance of '#' with '\0'
